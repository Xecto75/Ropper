...
const SCROLL_SPEED_START : float = 6.0
const SCROLL_SPEED_MAX   : float = 20.0
const SCROLL_ACCEL       : float = 0.5
...
func _anchor_chance_at(row: int, col: int) -> float:
	# Check 2 rings of neighbors — no anchors within 2 tiles of each other
	var checked : Array = hex_neighbors(row, col)
	for n : Vector2i in hex_neighbors(row, col):
		for n2 : Vector2i in hex_neighbors(n.x, n.y):
			if not checked.has(n2) and not (n2.x == row and n2.y == col):
				checked.append(n2)
	for n : Vector2i in checked:
		if n.x >= grid.size() or n.y >= grid[n.x].size():
			continue
		var t = grid[n.x][n.y]
